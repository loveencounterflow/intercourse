(function() {
  'use strict';
  var $, $async, CND, PATH, PD, assign, badge, collapse_text, copy, debug, declare, echo, has_full_signatures, help, info, isa, jr, lets, partition, rpr, select, size_of, type_of, types, urge, validate, warn, whisper;
  ...
  this.$add_headers = function(S) {
    ...
    return $((d, send) => {
      ...
      if ((m = d.value.match(ignore_re)) != null) {
        return send(PD.new_datom('^ignore', {
          value: copy(m.groups),
          $: d,
          ref: 'ah/1'
        }));
      ...
      if ((m = d.value.match(header_sig_re)) != null) {
        return send(PD.new_datom('^definition', {
          value: copy(m.groups),
          $: d,
          ref: 'ah/2'
        }));
...
(line #${lnr})`);
  ...
  this.definitions_from_path = function(path, settings) {
    return new Promise((resolve, reject) => {
      return this._read_definitions(PD.read_from_file(path), settings, (error, R) => {
        if (error != null) {
          return reject(error);
        }
        return resolve(R);
  ...
  this.definitions_from_text = function(text, settings) {
    return this._read_definitions(PD.new_value_source([text]), settings);
  };
